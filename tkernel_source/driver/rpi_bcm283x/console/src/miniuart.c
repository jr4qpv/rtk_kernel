...
/*
 *	@(#)miniuart.c	Console/Low-level serial I/O driver (driver) 2018/04/20
 *
...
#ifdef REMOTE_MODE
#include <opt/remote.h>			/* for _remote_??? */

//#define	REMOTE_MODE_LOG
#ifdef REMOTE_MODE_LOG
static volatile short *log = (short *)0x08000500;
static unsigned short logptr = 0;
#endif
#endif	/* #ifdef REMOTE_MODE */
...
#ifdef REMOTE_MODE
		// ﾒﾝﾃﾎﾟｰﾄ remote処理
		if ((i = _remote_rx(c, REMOTE_MODE)) < 0) {
#ifdef REMOTE_MODE_LOG
			if (i != -3)
				log[logptr++] = (i<<8) + c;
#endif
   ...
   if (iir == IS_SND) {				/*** 送信関係割り込み? ***/
	...
	if ( (lsts & LS_THRE) != 0				/* Tx Ready */
	  ...
	  && (li->flow.csflow == 0 || (li->msts & MS_CS) != 0)	/* CS ON */
		...
		while (i >= 0) {
			stat = InW(regMU_STAT);
			if (stat & STAT_TXFFUL) break;	/* Tx Full? */
			
			// ﾒﾝﾃﾎﾟｰﾄ remote処理
			if ((i = _remote_tx(REMOTE_MODE)) >= 0) {
#ifdef REMOTE_MODE_LOG
				log[logptr++] = i;
#endif
				OutW(regMU_IO, i);			// リモート文字を送信
				ENB_TXRXINT(li); /* Enable the snd/rcv interrupts */
				continue;
			}
		}
