...
int32_t R_SPIBSC_ExmodeInit(st_spibsc_cfg_t *spibsccfg)
{
    ...
    ret = sflash_spibsc_bsz_set();
    if (0 == ret)
    {
        ret = sflash_spibsc_dr_init(spibsccfg);
    }
...
int32_t R_SPIBSC_ExmodeSetting(st_spibsc_cfg_t *spibsccfg)
{
    ...
    if (0 == ret)
    {
        ...
        ret = sflash_spibsc_common_init(spibsccfg->udef_data_width,
                                        spibsccfg->udef_spbr,
                                        spibsccfg->udef_brdv);
        if (0 == ret)
        {
