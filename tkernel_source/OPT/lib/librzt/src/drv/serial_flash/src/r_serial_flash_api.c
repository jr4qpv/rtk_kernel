...
*                                     - R_SPIBSC_ByteProgram function modified(bugfix)
...
*              : -1 : error
* Limitation   : Arguments of this function must satisfy the following condition.
*                 addr_mode  : SPIBSC_OUTPUT_ADDR_24 or SPIBSC_OUTPUT_ADDR_32
******************************************************************************/
int32_t R_SPIBSC_EraseSector(uint32_t addr, uint32_t data_width, uint32_t addr_mode)
{
    ...
    if (0 == ret)
    {
        ...
        ret = sflash_spibsc_transfer_erase(addr, cmd, addr_mode);
        if (0 == ret)
        {
...
*              : -1 : error
* Limitation   : Arguments of this function must satisfy the following condition.
*                 addr       : Aligned on 4-byte boundary.
*                 size       : Multiple of 4.
*                 data_width : SPIBSC_1BIT or SPIBSC_4BIT
*                 addr_mode  : SPIBSC_OUTPUT_ADDR_24 or SPIBSC_OUTPUT_ADDR_32
******************************************************************************/
                             ...
                             uint32_t data_width, uint32_t addr_mode)
{
    ...
    if ( (size % 4) == 0 )
    {
        /* first page write, on page halfway */
        write_size = (SF_PAGE_SIZE - (addr & ((SF_PAGE_SIZE - 1u))));
        if (write_size > size)
        {
            write_size = size;
        }
        ...
        while ( (ret == 0) && (size > 0) )
        {
            ...
            if (0 == ret)
            {
                ret = (int32_t)sflash_spibsc_transfer_program_1(addr, cmd, addr_mode);
                if (0 == ret)
                {
                    ret = (int32_t)sflash_spibsc_transfer_program_2((uint32_t)SPIBSC_OUTPUT_SPID_32, data_width, (uint32_t *)buf, write_size);
                    if (0 == ret)
                    {
...
*                -1 : error
* Limitation   : Arguments of this function must satisfy the following condition.
*                 addr       : If data_width is SPIBSC_4BIT then addr must be
...
{
    return sflash_spibsc_transfer_single(regset);
}
/*******************************************************************************
...
static int32_t read_data_quad(uint32_t addr, uint8_t * buf, int32_t unit, int32_t addr_mode)
{
   ...
   /* ---- Command,Address,Dummy ---- */
    ...
    ret = sflash_spibsc_transfer_single(&g_spimd_reg); /* Command,Address */
    if (0 == ret)
    {
        /* ---- Data ---- */
        ...
        ret = sflash_spibsc_transfer_single(&g_spimd_reg);         /* Data input */
        if (0 == ret)
        {
            if ( 1 == unit )
            {
