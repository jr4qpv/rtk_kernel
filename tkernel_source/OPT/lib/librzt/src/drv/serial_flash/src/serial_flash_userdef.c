...
*                                     - modified dummy cycle comments
*              : 31.10.2017 1.3      4th Release
*                                     - No modification
*******************************************************************************/
#ifndef __TMONITOR_BUILD__	 /* Not T-Monitor build ? */
 #define	USE_TK_WAIT		1
#endif
...
#ifdef USE_TK_WAIT
#include <basic.h>
#include <tk/tkernel.h>			/* for tk_dly_tsk */
#endif
#include "platform.h"
#include "iodefine.h"
...
{
    uint8_t    st_reg = 0;		/* Set zero for warning */
    int32_t    ret;
        ...
        if (0 == ret)
        {
            ...
            if(0 == ret)
            {
                ...
                cf_reg = (cf_reg | (uint8_t)0x80);  /* Dummy cycle is 8cycle */
                                                    /* In the case of MX25L51245G and 32bit Address, 
                                                       2 dummy cycles are included in Address cycles. */
...
#if (SPIBSC_SFLASH_TYPE == TYPE_MT25QL)
	g_spimd_reg.cmd   = (uint32_t)0x85;                       /* Read Volatile Configuration Register */
#else
...
static int32_t write_status(uint8_t status, uint8_t config, uint32_t data_width)
{
 #if !(defined(__TMONITOR_BUILD__) && defined(__DEBUG_BUILD__))	/* [FIXME!] /////サイズ削減の為///// */
    int32_t    ret;
...
/*----------------------------------------------------------------------
#|History of "serial_flash_userdef.c"
#|-----------------------------------
#|* 2016/04/08	Renesasの｢SPIBSC｣ｻﾝﾌﾟﾙｺｰﾄﾞから修正。(by T.Yokobayashi)
#|* 2017/07/06	ｼﾘｱﾙﾌﾗｯｼｭ「TYPE_MT25QL」対応。
#|
*/
