...
// Push registers for nested interrupt Macro (inline asembler code)
//  - Need to insert at top of interrupt service
//  - Define __FPU_USED__ macro before include if FPU is used
...
#define NESTED_INTERRUPT_PUSH()\
    asm("push {r0, r1}");       /* Push work register to IRQ stack*/\
    asm("srsfd  sp!, #0x12");   /* Push LR, SPSR to IRQ stack*/\
\
    asm("cps  #0x1f");          /* Change to system mode*/\
\
    asm("and r1, sp, #4");      /* Adjust SP to 8 byte align*/\
    asm("sub sp, sp, r1");\
    asm("push {r1, lr}");       /* Push LR along with adjust value*/

// Pop registers for nested interrupt Macro (inline asembler code)
...
#define NESTED_INTERRUPT_POP()\
    asm("pop {r1, lr}");    /* Pop LR and SP adjusted value */\
    asm("add sp, sp, r1");  /* Restore SP before aligned */\
\
    asm("cps #0x12");       /* Change to IRQ mode */\
\
    asm("pop {r0, r1}");    /* Pop LR to r0 and SPSR to r1 */\
    asm("mov lr, r0");      /* Restore LR */\
    asm("msr spsr, r1");    /* Restore SPSR */\
\
    asm("pop {r0, r1}");    /* Pop work register from IRQ stack */
