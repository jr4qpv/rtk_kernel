...
#include "r_reset.h"
#include "_type_rzt_def.h"		/* for SPIBSC_SFLASH_TYPE,BSC_SDRAM_TYPE */
...
void bsc_init(void)
{
...
#if (BSC_SDRAM_TYPE == TYPE_MD56V62161M)
    BSC.SDCR.BIT.A2ROW = BSC_ROW_12_BIT;     // Row address is 12 bit
    BSC.SDCR.BIT.A2COL = BSC_COL_8_BIT;      // Column address is 8 bit
    ...
    BSC.SDCR.BIT.A3ROW = BSC_ROW_12_BIT;     // Row address is 12 bit
    BSC.SDCR.BIT.A3COL = BSC_COL_8_BIT;      // Column address is 8 bit
#elif (BSC_SDRAM_TYPE == TYPE_MT48LC16M16A2)
    BSC.SDCR.BIT.A2ROW = BSC_ROW_13_BIT;     // Row address is 13 bit
    BSC.SDCR.BIT.A2COL = BSC_COL_9_BIT;      // Column address is 9 bit
    BSC.SDCR.BIT.DEEP = BSC_DEEP_SELF;       // Deep power down mode
    BSC.SDCR.BIT.RFSH = BSC_RFSH_DONE;       // Reflesh is done
    BSC.SDCR.BIT.RMODE = BSC_RMODE_AUTO;     // Auto reflesh 
    BSC.SDCR.BIT.PDOWN = BSC_PDOWN_INVALID;  // Power down is invalid
    ...
    BSC.SDCR.BIT.A3COL = BSC_COL_9_BIT;      // Column address is 90 bit
#else
    BSC.SDCR.BIT.A2ROW = BSC_ROW_13_BIT;     // Row address is 13 bit
...
//	SPIBSC.SPBCR.BIT.SPBR = 1;		/* 75.0Mbps */
	SPIBSC.SPBCR.BIT.SPBR = 2;		/* 37.5Mbps */
#else
    SPIBSC.SPBCR.BIT.SPBR = 2;		/* 37.5Mbps */
#endif
    /*--------------------------------------*/
...
/*----------------------------------------------------------------------
#|History of "bus_init_serial_boot.c"
#|-----------------------------------
#|* 2016/04/22	Renesasの｢初期設定｣ｻﾝﾌﾟﾙｺｰﾄﾞから修正。(by T.Yokobayashi)
#|* 2016/04/22	spibsc_init()で、MSTPCRC9 のﾓｼﾞｭｰﾙｽﾄｯﾌﾟ解除(=0)する処理追加
#|* 2017/07/06	Add dummy-read for the module-stop state.
#|* 2017/07/31	SDRAM指定「BSC_SDRAM_TYPE==TYPE_MD56V62161M」に対応。
#|* 2018/03/23	SDRAM指定「BSC_SDRAM_TYPE==TYPE_MT48LC16M16A2」に対応。
#|* 2018/04/16	「BOARD_RZT1_RSK」に対応。
#|
*/
