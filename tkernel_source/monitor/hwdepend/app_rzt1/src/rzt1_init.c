...
void	rzt1_initialize(void)
{
	...
	/* Set PU7 to Input port pin (Using DIPSW4-6) */
	...
	/* Set P35 to Input port pin (Using NMI SW1) */
	PORT3.PDR.BIT.B5 = PORT_DIRECTION_HIZ;  // Initialize port settings
	PORT3.PMR.BIT.B5 = PORT_MODE_GENERAL;

	PORT3.PDR.BIT.B5 = PORT_DIRECTION_INPUT;  // Set port direction to intput
	return;
}
...
#|* 2019/10/08	RSK時、PU7(DIPSW4-6)は入力ピンに設定。
#|* 2019/10/09	P35(NMI SW)は入力ピンに設定。
#|
*/
